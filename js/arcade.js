 ...
 * Task: Use the logic_index to instantly grab the code and inject user properties.
 */
function assembleFromIndex(userSpark, library) {
    // Direct jump to the correct code blueprint
...
/**
 * Overall Objective: Create a Spark node using direct index mapping to the Type Library.
 * Task: Resolve the Spark instance by linking the user's specific properties to a Category Index.
 */
async function handleCreationNew(userPrompt, libraryIndex = null) {
    const library = await fetchArcadeCurrentTypes();
    
    // Resolve: If we don't have an index (e.g., manual prompt), find it first
    let targetIndex = libraryIndex;
    let userOverrides = {};

    if (targetIndex === null) {
        const resolution = resolveCategoryFromPrompt(userPrompt, library);
        if (resolution.is_custom) {
            // Treat as custom/LLM flow as discussed
            return await handleCustomLLMFlow(userPrompt);
        }
        // Find the index of the detected category
        targetIndex = library.findIndex(t => t.id === resolution.category_id);
        userOverrides = resolution.properties;
    }

    // Direct Access: No searching required once we have the index
    const typeNode = library[targetIndex];

    const sparkNode = {
        // We store the library index here instead of a complex hash
        logic_index: targetIndex, 
        properties: {
            ...typeNode.defaults,
            ...userOverrides
        },
        template_type: typeNode.group, // Keep for UI grouping/filtering
 ...
 * Task: Extract a schema of parameters and defaults to define the Category's property map.
 */
...
function distillSparkCodeToCache(sparkNode, currentLibrary) {
    ...
    // 1. ISOLATE: Extract the core logic from the script tags
    const scriptMatch = rawCode.match(/<script>([\s\S]*?)<\/script>/i);
    logic = scriptMatch ? scriptMatch[1].trim() : rawCode;

    // 2. IDENTIFY & FILTER: Find variable declarations with significance heuristics
    const configPattern = /(const|let|var)\s+(\w+)\s*=\s*([^;]+);/g;
    const foundParams = {};
    
    // Create the template by replacing static assignments with placeholders
    const templateWithVars = logic.replace(configPattern, (match, type, name, value) => {
        const trimmedVal = value.trim();
        
        // HEURISTIC: Only parameterize if it's a simple number/string 
        // AND it's likely a simulation property (Upper Case or common Keywords)
        ...
        if (isParamCandidate) {
            // Clean the value (remove quotes) to store as a clean default
    ...
    // 3. PARAMETERIZE COUNTS: Identify hardcoded loop boundaries
    ...
    // 4. GENERATE PROPERTY MAP: Create regex patterns for natural language extraction
    const parameterMap = Object.keys(foundParams).reduce((map, key) => {
        // Generates a regex that looks for the key name followed by the value
        map[key] = `(?:${key}(?:\\s(?:is|of|at))?\\s)?([-#\\w.]+)`;
        return map;
    }, { count: "(\\d+)(?=\\s*(?:objects|items|units)?)" });

    // 5. PREPARE TYPE NODE: This is the "Class definition"
    const newTypeEntry = {
        ...
        defaults: {
            ...foundParams,
            count: foundParams.count || 3
        }
    };

    // 6. INDEX ASSIGNMENT: Append to global library
    const newIndex = currentLibrary.push(newTypeEntry) - 1;

    // Return everything handleCreation needs to finish the job
    return {
        newIndex: newIndex,
        typeData: newTypeEntry, // Used to update arcade-current-types
        extractedProperties: newTypeEntry.defaults // Used to populate the user's spark node
    };
}

/*
 * Generates a stable structural hash for the logic block.
 * Strips comments and whitespace so formatting changes don't break the hash.
 */
function generateInternalHash(str) {
    // 1. Structural Sanity: Remove comments and all whitespace
    const structuralBase = str.replace(/\/\/.*|\/\*[\s\S]*?\*\/|\s+/g, "");
    
    let hash = 0;
    for (let i = 0; i < structuralBase.length; i++) {
        const char = structuralBase.charCodeAt(i);
        hash = ((hash << 5) - hash) + char;
        hash = hash & hash; // Convert to 32bit integer
    }
    return `logic_${Math.abs(hash).toString(16)}`;
}
