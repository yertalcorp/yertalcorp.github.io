...
console.log(`%c YERTAL ARCADE LOADED | ${new Date().toLocaleDateString()} @19:26:00 `, "background: var(--bg-color); color: var(--branding-color); font-weight: bold; border: 1px solid var(--branding-color); padding: 4px;");
...
function shapeAiPrompt(rawPrompt, count, mode, currentName, promptTypeObject) {
    ...
    if (isSource) {
        ...
        instructions = `Write a visually stunning, fully working HTML/Javascript application with gradient colors and 3D objects.
                     - Format: JSON object {"name", "code", "thumbnail"} and name has maximum 3 words. The thumbnail must be a publicly available and high-resolution image URL that represents the application.`;
    } 
