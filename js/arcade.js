...
console.log(`%c YERTAL ARCADE LOADED | ${new Date().toLocaleDateString()} @ 21:32:00 `, "background: var(--bg-color); color: var(--branding-color); font-weight: bold; border: 1px solid var(--branding-color); padding: 4px;");
...
function shapeAiPrompt(rawPrompt, count, mode, currentName, promptTypeObject) {
    ...
    const instructions = isSource ? 
        ...
        `Write a visually stunning, bright, fully working HTML/Javascript application with gradient colors and 3D objects.
        -Format: JSON object {"name", "code"} and name has maximum 3 words.`;

    return `
        ${rawPrompt}. Follow this model: ${promptTypeObject.name} and use best practices.
        ${instructions}.
