...
function genSparkImage(sparkImageFromDB) {
    // 1. Check if the field is empty or undefined
    if (!sparkImageFromDB) {
        return 'assets/thumbnails/default.jpg';
    }

    // 2. Check if the string is a Base64/Data URI (starts with "data:image/")
    // This identifies the "micro-poster" generated by our converter.
    if (sparkImageFromDB.startsWith('data:image/')) {
        return sparkImageFromDB;
    }

    // 3. Otherwise, return as a standard URL/Path
    // This handles your existing hardcoded paths or external links.
    return sparkImageFromDB;
}
