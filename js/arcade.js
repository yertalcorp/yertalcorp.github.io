...
console.log(`%c YERTAL ARCADE LOADED | ${new Date().toLocaleDateString()} @08:17:00 `, "background: var(--bg-color); color: var(--branding-color); font-weight: bold; border: 1px solid var(--branding-color); padding: 4px;");
...
function shapeAiPrompt(rawPrompt, count, mode, currentName, promptTypeObject) {
    ...
    if (isSource) {
            instructions = `- Research specific items that match the task.
                         ...
                         - Format: JSON array [{"name", "url", "thumbnail"}] and name has maximum 3 words. The thumbnail is a URL for an existing and high resolution image.`;
    } else {
        ...
        instructions = `Write a visually stunning, fully working HTML/Javascript application with gradient colors and 3D objects.
                     - Format: JSON object {"name", "code", "thumbnail"} and name has maximum 3 words. The thumbnail is a url for an existing image that represents the application.`;
    } 
